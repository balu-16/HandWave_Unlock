...
def unlock_laptop():
    try:
        logger.info("Gesture matched! Unlocking in 2 seconds...")
        time.sleep(2)
        ...
        logger.info("Laptop unlock command sent successfully")
    except Exception as e:
...
def process_gestures():
    ...
    try:
        ...
        signal_frame = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(signal_frame, "Camera Started", (240, 240),
...
def generate_frames():
    ...
    default_frame = np.ones((480, 640, 3), dtype=np.uint8) * 245
